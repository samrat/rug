...
use crate::database::{Object, Blob, Tree, Entry};
...
pub fn commit_command<I,O,E>(mut ctx: CommandContext<I,O,E>) -> Result<(), String>
    ...
    repo.index.load()
        .expect("loading .git/index failed");
    ...
    let author_name = ctx.env.get("GIT_AUTHOR_NAME")
        .expect("GIT_AUTHOR_NAME not set");
    let author_email = ctx.env.get("GIT_AUTHOR_EMAIL")
        .expect("GIT_AUTHOR_EMAIL not set");

    let author = Author { name: author_name.to_string(),
                          email: author_email.to_string() };

    let mut commit_message = String::new();
    ctx.stdin.read_to_string(&mut commit_message)
        .expect("reading commit from STDIN failed");

    let commit = Commit::new(&parent, root.get_oid(), author, commit_message);
    repo.database.store(&commit)
        .expect("writing commit failed");
    ...
    for arg in &ctx.args[2..] {
        let path = match Path::new(arg).canonicalize() {
            ...
            Err(_) => {
                ...
                return Err(
                    format!("fatal: pathspec '{:}' did not match any files\n", arg)
                );
